...
const long int maxTimeStep = 1000000;
...
int main() {
  ...
  for (long int t = 0; t < maxTimeStep; t++) {
    ...
    // Collision and Reversing 
    // Collision
    // Calculating the physical variables, f_eq
    ...
    for (int i = 1; i < nx-1; i++) {
      ...
      f[i][1][6] = f[i+1][0][6];
      // Top wall
      f[i][ny-2][4] = f[i][ny-1][4];
      f[i][ny-2][8] = f[i-1][ny-1][8];
      f[i][ny-2][7] = f[i+1][ny-1][7];
    }
    for (int j = 2; j < ny-2; ++j) {
      ...
      f[1][j][8] = f[0][j+1][8];
      // Right Wall
      f[nx-2][j][3] = f[nx-1][j][3];
      f[nx-2][j][6] = f[nx-1][j-1][6];
      f[nx-2][j][7] = f[nx-1][j+1][7];
    }
    // Calculating f(t+dt)
    for (int i = 0; i < nx; i++) {
      for (int j = 0; j < ny; j++) {
